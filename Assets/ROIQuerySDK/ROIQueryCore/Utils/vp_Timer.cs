...
//#define DEBUG	// uncomment to display timers in the Unity Editor Hierarchy
    ...
    public class vp_Timer : MonoBehaviour
    {
        ...
        private static readonly List<Event> m_Active = new List<Event>();
        private static readonly List<Event> m_Pool = new List<Event>();

        private static Event m_NewEvent;
        private static int m_EventCount;

        // variables for the Update method
        private static int m_EventBatch;
        private static int m_EventIterator;
        ...
        private void Awake()
        {
            if (!WasAddedCorrectly) Destroy(this);
        }
        ...
        private void Update()
        {
            ...
            while (m_Active.Count > 0 && m_EventBatch < MaxEventsPerFrame)
            {
                ...
                if (Time.time >= m_Active[m_EventIterator].DueTime || // time is up
                    m_Active[m_EventIterator].Id == 0) // event has been canceled ('Execute' will kill it)
                {
                ...
                {
                    // handle pausing
                    if (m_Active[m_EventIterator].Paused)
        ...
        {
            for (var t = m_Active.Count - 1; t > -1; t--) m_Active[t].Id = 0;
        }
        ...
        {
            for (var t = m_Active.Count - 1; t > -1; t--)
                if (m_Active[t].MethodName == methodName)
                    m_Active[t].Id = 0;
        }
        ...
        {
            m_MainObject.name = "Timers (" + m_Active.Count + " / " + (m_Pool.Count + m_Active.Count) + ")";
        }
#endif
        ...
        private class Event
        {
	        ...
	        public void StoreCallingMethod()
            {
                ...
                var result = "";
                var declaringType = "";
                for (var v = 3; v < stackTrace.FrameCount; v++)
                {
                    var stackFrame = stackTrace.GetFrame(v);
            ...
            {
                var msg = "Error: (vp_Timer.Event) " + message;
#if (UNITY_EDITOR && DEBUG)
                msg += MethodInfo;
#endif
                Debug.LogError(msg);
        ...
        public class Handle
        {
            private Event m_Event; // timer we're pointing at
            private float m_FirstDueTime; // the initial execution delay of the event
            private int m_Id; // id associated with timer upon creation of this handle
            ...
            /// </summary>
            public float Delay => Mathf.Round((m_FirstDueTime - TimeOfInitiation) * 1000.0f) / 1000.0f;
            ...
            public float DurationTotal
            {
                get
                {
                    if (Active)
                        return Delay +
                               IterationsTotal * (IterationsTotal > 1 ? Interval : 0.0f);
                    return 0.0f;
                }
            }
            ...
            /// </summary>
            public int IterationsTotal { get; private set; } = 1;
            ...
            public int Id
            {
                ...
                set
                {
                    ...
                    if (m_Event == null)
                        Debug.LogError("Error: (vp_Timer.Handle) Failed to assign event with Id '" + m_Id + "'.");

                    // store some initial event info
                    IterationsTotal = m_Event.Iterations;
            ...
            ///     returns the name of the scheduled method, or 'delegate'
            /// </summary>
            public string MethodName => m_Event.MethodName;

            /// <summary>
            ///     returns the name of the scheduled method along with any arguments,
            ///     plus a stack trace (if compiling with the DEBUG define)
            /// </summary>
            public string MethodInfo => m_Event.MethodInfo;
