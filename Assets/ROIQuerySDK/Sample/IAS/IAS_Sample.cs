...
public class IAS_Sample : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        buttons1[0].onClick.AddListener(delegate { ROIQueryIASAPI.ReportToShow(seq, placement, entrance); });
        buttons1[1].onClick.AddListener(delegate { ROIQueryIASAPI.ReportShowSuccess(seq, placement, entrance); });
        buttons1[2].onClick.AddListener(delegate
        {
            ROIQueryIASAPI.ReportShowFail(seq, placement, errorCode, errorMsg, entrance);
        });
        buttons1[3].onClick.AddListener(delegate
        {
            ROIQueryIASAPI.ReportSubscribe(seq, placement, sku, orderId, price, currency, entrance);
            //用法相同
        });
        buttons1[4].onClick.AddListener(delegate
        {
            ROIQueryIASAPI.ReportSusbscribeSuccess(seq, placement, sku, orderId, originalOrderId, price, currency,
                entrance);
        });
        buttons1[5].onClick.AddListener(delegate
        {
            ROIQueryIASAPI.ReportSubscribeFail(seq, placement, sku, orderId, originalOrderId, price, currency,
